...
TR.updateBeatsSelect = function(inst) {
  ...
  if (naturalBeats === defaultBeats) {
    sel.innerHTML = '<option value="' + defaultBeats + '" selected>周期同期</option>';
    sel.disabled = true;
  } else {
    var syncOpt = '<option value="' + defaultBeats + '"' + (wasAsync ? '' : ' selected') + '>周期同期</option>';
    var asyncOpt = '<option value="' + naturalBeats + '"' + (wasAsync ? ' selected' : '') + '>拍同期</option>';
