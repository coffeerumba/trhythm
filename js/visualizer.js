...
function vizCell(x, y, r, g, b) {
  ...
  vizCtx.fillRect(Math.floor(x / CELL) * CELL, Math.floor(y / CELL) * CELL, CELL - 1, CELL - 1);
}
...
/* ── Hit Event ── */
TR.vizOnHit = function(key) {
  vizAddSignal(key);
};

/* ── Main Loop ── */
function vizFrame() {
  vizCtx.fillStyle = 'rgb(' + VIZ_BG[0] + ',' + VIZ_BG[1] + ',' + VIZ_BG[2] + ')';
  vizCtx.fillRect(0, 0, vizW, vizH);
  vizUpdateSnakes();
  vizDrawTraces();
  vizDrawSnakes();
  vizAnimId = requestAnimationFrame(vizFrame);
}

/* ── Resize ── */
TR.vizResize = function() {
  var wrap = document.getElementById('viz-wrap');
  vizW = Math.floor(wrap.clientWidth / 2);
  vizH = Math.floor(vizW * 9 / 16);
...
TR.vizStart = function() { if (!vizAnimId) vizAnimId = requestAnimationFrame(vizFrame); };
TR.vizStop = function() { if (vizAnimId) { cancelAnimationFrame(vizAnimId); vizAnimId = null; } }; // Currently unused; kept for future use (e.g., tab visibility optimization)
